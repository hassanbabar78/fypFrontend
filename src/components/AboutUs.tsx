...
const AboutUs = () => {
  ...
  return (
    <section id="about-us" className="relative py-20 px-6 lg:px-8 bg-background overflow-hidden">
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-20 -right-20 w-60 h-60 bg-primary2/5 rounded-full blur-3xl"></div>
        <div className="absolute -bottom-20 -left-20 w-60 h-60 bg-primary2/5 rounded-full blur-3xl"></div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-foreground mb-6">
            About{" "}
            <span className="text-transparent bg-clip-text bg-primary2">Us</span>
          </h2>
          <p className="text-xl text-muted-foreground lg:text-2xl max-w-4xl mx-auto leading-relaxed">
        ...
        <div ref={statsRef} className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-16 max-w-4xl mx-auto">
          {stats.map((stat, index) => (
            <div
              key={index}
              className="text-center p-6 rounded-2xl bg-secondary backdrop-blur-sm border border-border transition-all duration-300"
            >
              <div
                className={`text-2xl lg:text-3xl font-bold text-primary2 mb-2 transition-all duration-500 ${!isVisible ? 'opacity-0' : hasAnimated && counts[index] === 0 ? 'opacity-0' : 'opacity-100'
                  }`}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {features.map((feature, index) => (
            <Card
              key={index}
              className="group relative overflow-hidden border bg-background border-border transition-all duration-500 cursor-pointer shadow-[0_0_30px_-5px_hsl(var(--primary-blue))] shadow-primary-blue/10"
            >
              <div className="absolute inset-0 bg-primary2/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              ...
              <CardHeader className="relative z-10 pb-4">
                <div className="flex items-center gap-4 mb-4">
                  <div className="p-3 rounded-xl bg-[#346F84] text-primary2-foreground shadow-lg group-hover:shadow-primary2/20 transition-all duration-500 ease-out group-hover:-translate-y-2 group-hover:translate-x-1 group-hover:rotate-9">
                    {feature.icon}
                  </div>
                ...
                <CardDescription className="text-muted-foreground text-lg font-medium">{feature.stats}</CardDescription>
              </CardHeader>
              ...
              <CardFooter className="relative z-10 pt-4 border-t border-border group-hover:border-primary-blue/30 transition-colors duration-300">
                <div className="flex items-center justify-between w-full">

                  <span className="text-[15px] text-muted-foreground transition-colors duration-400 inline-flex items-center">
                    Learn more
                    <span className="ml-2 text-[15px] text-muted-foreground transition-all duration-400 group-hover:translate-x-2">
                      →
                    </span>
                  </span>
                  <div className="w-2 h-2 bg-[#346F84] rounded-full transition-transform duration-300"></div>
                </div>
              </CardFooter>
            </Card>
          ))}
        ...
        <div className="text-center mt-16">
          <div className="inline-flex flex-col sm:flex-row gap-4 items-center p-5 rounded-2xl bg-gradient-to-r from-[#346F84]/10 to-[#346F84]/20 backdrop-blur-sm border border-primary2/20">
            <div className="text-left">
              <h3 className="text-xl font-bold text-foreground mb-2">Ready to Secure Your Digital Identity?</h3>
              <p className="text-muted-foreground text-[15px]">
                Join thousands of organizations trusting PKIChain for their domain validation needs.
              </p>
            </div>
            <button className="px-7 py-3 bg-[#346F84] text-primary2-foreground rounded-xl font-semibold shadow-lg hover:shadow-primary-blue/15 hover:scale-105 transition-all duration-400 whitespace-nowrap">
